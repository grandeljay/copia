...
{
    if (
        !magnaInstalled(($opts & MAGNA_WITHOUT_DB_INSTALL) == MAGNA_WITHOUT_DB_INSTALL)
        || !(
...
function defineMagnalisterDir()
{
    ...
    // called from admin
    } elseif ((MAGNA_IN_ADMIN == true) && file_exists('includes/magnalister/')) {
    ...
    } elseif ((MAGNA_IN_ADMIN == false) && defined('DIR_FS_ADMIN') && file_exists(DIR_FS_ADMIN . 'includes/magnalister/')) {
    ...
    // fallback 2
    } elseif ((MAGNA_IN_ADMIN == false) && file_exists(dirname(__FILE__) . '/admin/includes/magnalister/')) {
...
function magnaInstalled($woDBCheck = false)
{
    ...
    while (false !== ($filename = readdir($dirhandle))) {
        if (!preg_match('/^[0-9]*\.sql\.php$/', $filename)) {
            continue;
        }
        $sqlFiles[] = $filename;
    }
...
{
    if (
        ($currentClientVersion = fileGetContents(
...
function magnaDetermineCurrentClientVersion()
{
    ...
    do {
        if (
            !is_array($cCVDB)
             || !array_key_exists('DATETIME', $cCVDB)
        ) {
            break;
        }
        if (
            ($cCVDB['DATETIME'] < (time() - 10 * 60))
        ...
        if ($cCVDB['DATETIME'] < (time() - 60 * 60 * 24)) {
            break;
        }
        $cCV = $cCVDB;
...
function magnaCallbackRun()
{
    ...
    if (
        MagnaDB::gi()->recordExists(TABLE_MAGNA_CONFIG, array(
        'mpID' => 0,
...
if (MAGNA_CALLBACK_MODE == 'STANDALONE') {
    ...
    if (!in_array('application_top.php', preg_replace("/\/.*\//", "", get_included_files()))) {
        ...
        if (
            !file_exists(dirname(__FILE__) . '/GambioCore/Application/Kernel/Bootstrapper/Language/LanguageFinder.php')
    ...
    if (
        (dirname($_SERVER['SCRIPT_FILENAME']) . '/' == DIR_FS_DOCUMENT_ROOT) #browser
