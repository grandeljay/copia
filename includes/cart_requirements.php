...
if (isset($_SESSION['err_max_prod'])) {
    ...
    for ($i = 0, $n = sizeof($products); $i < $n; $i++) {
        ...
        } elseif (isset($_SESSION['err_max_prod']) && (int)$_GET['max_prod_id'] == $products[$i]['id']) {
            $emsg = sprintf(MAX_PROD_QTY_EXCEEDED, $products[$i]['name']);
            $error_msg[] = array(E_MSG => $emsg);
        }
    }
...
foreach (auto_include(DIR_FS_CATALOG . 'includes/extra/shopping_cart/cart_requirements/', 'php') as $file) {
    require_once($file);
}
