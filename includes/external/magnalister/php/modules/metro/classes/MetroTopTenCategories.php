...
class MetroTopTenCategories extends TopTen {
    ...
    public function getTopTenCategories($sType, $sGetCatPathFunc = 'getMPCategoryPath') {
        ...
        if (empty($aTopTenCat)) {
            $aTopTenCat = MagnaDB::gi()->fetchArray('
				SELECT DISTINCT MpIdentifier
				  FROM '.TABLE_MAGNA_METRO_VARIANTMATCHING.'
        ...
        $oDCM = new MetroCategoryMatching();

        $sGetCatPathFunc = 'getMetroCategoryPath';
        $aTopTenCatIds = array();
        foreach ($aTopTenCat as $iCatId) {
            ...
            if (strpos($aTopTenCatIds[$iCatId], '"invalid"') !== false) {
                ...
                // no mpid
                MagnaDB::gi()->query('
					UPDATE '.TABLE_MAGNA_METRO_PREPARE.'
					   SET '.$sType.' = 0
					 WHERE '.$sType.' = "'.$iCatId.'"
				');
            }
