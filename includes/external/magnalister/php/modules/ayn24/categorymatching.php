...
if (array_key_exists('saveMatching', $_POST)) {
	MagnaDB::gi()->query('
		REPLACE INTO '.TABLE_MAGNA_AYN24_CATEGORYMATCHING.'
...
if ((array_key_exists('unprepare', $_POST)) && (!empty($_POST['unprepare']))) {
	...
	if (!empty($pIDs)) {
		foreach ($pIDs as $pID) {
			...
			MagnaDB::gi()->delete(TABLE_MAGNA_AYN24_CATEGORYMATCHING, $where);
			MagnaDB::gi()->delete(TABLE_MAGNA_SELECTION, array(
				'pID' => $pID,
				'mpID' => $_MagnaSession['mpID'],
			    'selectionname' => $prepareSetting['selectionName'],
...
if (isset($_POST['prepare']) || (isset($_GET['where']) && ($_GET['where'] == 'catMatchView'))) {
	require_once(DIR_MAGNALISTER_MODULES.'ayn24/catmatch/Ayn24CatMatcher.php');
	$cM = new Ayn24CatMatcher($prepareSetting);
	echo $cM->run();
} else {
	require_once(DIR_MAGNALISTER_MODULES.'ayn24/catmatch/Ayn24PrepareCategoryView.php');
	$pV = new Ayn24PrepareCategoryView($current_category_id, $prepareSetting, $_GET['sorting'], $_POST['tfSearch']); /* $current_category_id is a global variable from xt:Commerce */
