 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: Ayn24PrepareCategoryView.php 4688 2014-10-08 13:06:57Z miguel.heredia $
 *
...
class Ayn24PrepareCategoryView extends SimpleCategoryView {
	...
	public function getAdditionalHeadlines() {
		return '
			<td class="lowestprice">'.ML_AYN24_LABEL_CATEGORY.'</td>
			<td class="matched">'.ML_AYN24_LABEL_PREPARED.'</td>';
	}
	...
	public function getAdditionalCategoryInfo($cID, $data = false) {
			...
			if (getDBConfigValue('general.keytype', '0') == 'artNr') {
				$query = '
					SELECT COUNT(DISTINCT pa.products_id) as itemsCount,
					  ...
					  FROM '.TABLE_PRODUCTS.' p, '.TABLE_MAGNA_AYN24_CATEGORYMATCHING.' pa
				...
				$query = '
					SELECT COUNT(DISTINCT products_id) as itemsCount,
					  ...
					  FROM '.TABLE_MAGNA_AYN24_CATEGORYMATCHING.'
		...
		if ($itemsApplied !== false) {
			if ($itemsApplied['itemsCount'] == 0) {
				...
				return $html.'
					<td title="'.ML_AYN24_LABEL_CATMATCH_NOT_PREPARED.'">'.
						html_image(DIR_MAGNALISTER_WS_IMAGES . 'status/grey_dot.png', ML_AYN24_LABEL_CATMATCH_NOT_PREPARED, 9, 9).
					'</td>';
			}
			if ($itemsApplied['incompleteCount'] == $totalItems) {
				/* Alle Artikel in Kategorie unvollstaendig beantragt */
				return $html.'
					<td title="'.ML_AYN24_LABEL_CATMATCH_PREPARE_INCOMPLETE.'">'.
						html_image(DIR_MAGNALISTER_WS_IMAGES . 'status/red_dot.png', ML_AYN24_LABEL_CATMATCH_PREPARE_INCOMPLETE, 9, 9).
					'</td>';
			}
			if (($itemsApplied['itemsCount'] == $totalItems) && ($itemsApplied['incompleteCount'] == 0)) {
				/* Alle Artikel in Kategorie beantragt */
				return $html.'
					<td title="'.ML_AYN24_LABEL_CATMATCH_PREPARE_COMPLETE.'">'.
						html_image(DIR_MAGNALISTER_WS_IMAGES . 'status/green_dot.png', ML_AYN24_LABEL_CATMATCH_PREPARE_COMPLETE, 9, 9).
					'</td>';
			}
			if ($itemsApplied['itemsCount'] > 0) {
				...
				return $html.'
					<td title="'.ML_AYN24_LABEL_CATMATCH_PREPARE_INCOMPLETE.'">'.
						html_image(DIR_MAGNALISTER_WS_IMAGES . 'status/yellow_dot.png', ML_AYN24_LABEL_CATMATCH_PREPARE_INCOMPLETE, 9, 9).
					'</td>';
			}
		}
		return $html.'
		...
		if ($a !== false) {
			if ($a['mp_category_id'] != '') {
				return '
					...
					<td>'.html_image(DIR_MAGNALISTER_WS_IMAGES . 'status/green_dot.png', ML_AYN24_LABEL_CATMATCH_PREPARE_COMPLETE, 9, 9).'</td>';
			} else {
				return '
					<td>'.$this->renderCatBlock($a).'</td>
					<td>'.html_image(DIR_MAGNALISTER_WS_IMAGES . 'status/red_dot.png', ML_AYN24_LABEL_CATMATCH_PREPARE_INCOMPLETE, 9, 9).'</td>';
			}
		}
		return '
			<td>&mdash;</td>
			<td>'.html_image(DIR_MAGNALISTER_WS_IMAGES . 'status/grey_dot.png', ML_AYN24_LABEL_CATMATCH_NOT_PREPARED, 9, 9).'</td>';
	}
