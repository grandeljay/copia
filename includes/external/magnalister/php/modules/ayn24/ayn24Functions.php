 ...
 *
 * -----------------------------------------------------------------------------
 * $Id: ayn24Functions.php 2332 2013-04-04 16:12:19Z derpapst $
 *
...
function magnaAyn24ProcessCheckinResult($result, $mpID) {
...
function magnaAyn24UpdateItems($mpID, $data) {
	magnaUpdateItems($mpID, $data, false, 'magnaAyn24ProcessCheckinResult');
}

function updateAyn24InventoryByEdit($mpID, $updateData) {
	...
	magnaAyn24UpdateItems($mpID, array($updateItem));
}

function updateAyn24InventoryByOrder($mpID, $boughtItems, $subRelQuant = true) {
	if (in_array(getDBConfigValue($_Marketplace.'.stocksync.tomarketplace', $mpID), array('no', 'auto'))) {
		return;
	}
	$data = genericInventoryUpdateByOrder($mpID, $boughtItems, $subRelQuant);
	#echo print_m($data, '$data');
	magnaAyn24UpdateItems($mpID, $data);
}
