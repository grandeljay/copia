...
class MLProductListDependencyFyndiqPrepareFormAction extends MLProductListDependency {
	public function getActionBottomLeftTemplate() {
		return 'fyndiqprepareformleft';
	}
	
	public function getActionBottomRightTemplate() {
		return 'fyndiqprepareformright';
	}
	...
	protected function resetDescription() {
		...
		if (!empty($pIDs)) {
			...
			$oFyndiqSaver = new FyndiqProductSaver($this->aMagnaSession);
			foreach ($pIDs as $pID) {
				$oFyndiqSaver->resetProductProperties($pID);
				MagnaDB::gi()->delete(TABLE_MAGNA_SELECTION, array(
					'pID' => $pID,
					'mpID' => $this->getMagnaSession('mpID'),
					'selectionname' => $this->getConfig('selectionname'),
