...
abstract class MLProductListFyndiqAbstract extends MLProductList {
	...
	protected function getPrepareData($aRow, $sFieldName = null) {
		if (!isset($this->aPrepareData[$aRow['products_id']])) {
			$this->aPrepareData[$aRow['products_id']] = MagnaDB::gi()->fetchRow("
				SELECT *
				  FROM ".TABLE_MAGNA_FYNDIQ_PROPERTIES."
	...
	protected function getFyndiqPrice($aRow) {
		$fPrice = $this->getPrepareData($aRow);
		if (empty($fPrice)) {
			return '&mdash;';
		}

		$price = $this->getPrice()->setFinalPriceFromDB($aRow['products_id'], $this->aMagnaSession['mpID']);
		$price->setPrice($price->getPrice() + $fPrice['ShippingCost']);
