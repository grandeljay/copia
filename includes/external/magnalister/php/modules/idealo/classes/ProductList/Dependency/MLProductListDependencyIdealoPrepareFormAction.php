...
class MLProductListDependencyIdealoPrepareFormAction extends MLProductListDependency {
    public function getActionBottomLeftTemplate() {
        return 'idealoprepareformleft';
    }

    public function getActionBottomRightTemplate() {
        return 'idealoprepareformright';
    }
    ...
    protected function unprepare() {
        ...
        if (!empty($aPIDs)) {
            foreach ($aPIDs as $sPID) {
                ...
                ;
                $where['mpID'] = $this->getMagnaSession('mpID');
                MagnaDB::gi()->delete(TABLE_MAGNA_IDEALO_PROPERTIES, $where);
    ...
    protected function resetDescription() {
        ...
        if (!empty($pIDs)) {
            require_once(DIR_MAGNALISTER_MODULES . 'idealo/classes/IdealoProductSaver.php');
            $oIdealoSaver = new IdealoProductSaver($this->aMagnaSession);
            foreach ($pIDs as $pID) {
                $oIdealoSaver->resetProductProperties($pID);
                MagnaDB::gi()->delete(TABLE_MAGNA_SELECTION, array(
                    'pID' => $pID,
                    'mpID' => $this->getMagnaSession('mpID'),
                    'selectionname' => $this->getConfig('selectionname'),
