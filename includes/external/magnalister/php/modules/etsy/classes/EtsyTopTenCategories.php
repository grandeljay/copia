...
class EtsyTopTenCategories extends TopTen {
	...
	public function getTopTenCategories($sType, $sGetCatPathFunc = 'getMPCategoryPath'){
		...
		if (empty($aTopTenCat)) {
			$aTopTenCat = MagnaDB::gi()->fetchArray('
				SELECT DISTINCT MpIdentifier
				  FROM '.TABLE_MAGNA_ETSY_VARIANTMATCHING.'
		...
		$oDCM = new EtsyCategoryMatching();

		$sGetCatPathFunc = 'getEtsyCategoryPath';
		$aTopTenCatIds = array();
		foreach ($aTopTenCat as $iCatId) {
			...
			if (strpos($aTopTenCatIds[$iCatId], '"invalid"') !== false) {
				...
				// no mpid
				MagnaDB::gi()->query('
					UPDATE '.TABLE_MAGNA_ETSY_PREPARE.'
					   SET '.$sType.' = 0
					 WHERE '.$sType.' = "'.$iCatId.'"
				');
			}
