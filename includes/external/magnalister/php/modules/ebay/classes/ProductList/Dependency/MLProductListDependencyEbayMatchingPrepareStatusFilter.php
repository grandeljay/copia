...
class MLProductListDependencyEbayMatchingPrepareStatusFilter extends MLProductListDependencyPrepareStatusFilter {

	protected function getPrepareCondition() {
		return array(
			'failed' => " AND (ePID IS NULL OR ePID = '')",
			'prepared' => " AND ePID IS NOT NULL
					AND ePID <> '' ",
			'notprepared' => "",
		);
	}
