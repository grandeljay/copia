...
class TradoriaPrepare extends MagnaCompatibleBase
{
	...
	protected function saveMatching()
	{
		...
		require_once(DIR_MAGNALISTER_MODULES . 'tradoria/classes/TradoriaProductSaver.php');

		$oProductSaver = new TradoriaProductSaver($this->resources['session']);
    ...
    protected function saveMatchingAttributes($oProductSaver, $isSinglePrepare)
    {
		...
		$oProductSaver->aErrors = array_merge($oProductSaver->aErrors,
			TradoriaHelper::gi()->saveMatching($sIdentifier, $matching, $savePrepare, true, $isSinglePrepare, $variationThemeAttributes));
    ...
    protected function deleteMatching()
    {
		...
		foreach ($pIDs as $pID) {
			...
			MagnaDB::gi()->delete(TABLE_MAGNA_TRADORIA_PREPARE, $where);
			MagnaDB::gi()->delete(TABLE_MAGNA_SELECTION, array(
				'pID' => $pID,
				'mpID' => $this->mpID,
				'selectionname' => $this->prepareSettings['selectionName'],
    ...
    public function process()
    {
		if (   isset($_POST['request'])
			...
			$product = array(
				'Id'		=> $_POST['productID'],
				'Results'	=> TradoriaHelper::SearchOnTradoria($sSearch, $sType)
			);
	...
	protected function getSelectedProductsCount()
	{
		$query = '
			...
			LEFT JOIN ' . TABLE_MAGNA_TRADORIA_PREPARE . ' p on p.mpID = s.mpID and p.products_id = s.pID
    ...
    private function insertAutoMatchProduct()
    {
		...
		foreach ($products as $product) {
			$searchResults = TradoriaHelper::SearchOnTradoria($product['EAN'], 'EAN');

			if (   $searchResults === false
				|| (is_array($searchResults) && count($searchResults) === 0)
			) {
				$searchResults = TradoriaHelper::SearchOnTradoria($product['Title'], 'Title');
			}
