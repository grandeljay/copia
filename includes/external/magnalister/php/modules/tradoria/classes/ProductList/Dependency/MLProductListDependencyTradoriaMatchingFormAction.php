...
class MLProductListDependencyTradoriaMatchingFormAction extends MLProductListDependency {
	public function getActionBottomLeftTemplate(){
		return 'tradoriamatchingformleft';
	}
	
	public function getActionBottomRightTemplate(){
		return 'tradoriamatchingformright';
	}
	
	public function getHeaderTemplate() {
		return 'tradoriamatchingform';
	}
		...
		if (!empty($pIDs)) {
			...
			if ( MLProduct::gi()->hasMasterItems()) {
				...
				", false), true) as $iMaster) {
					$aMaster = MLProduct::gi()->setLanguage(getDBConfigValue('tradoria.lang', $this->getMagnaSession('mpID'), $_SESSION['magna']['selected_language']))->getProductById($iMaster);
					$aVariants = array();
					...
					if(
						empty($aVariants) 
						|| 0 == MagnaDb::gi()->fetchOne("SELECT COUNT(".$sKeyType.") FROM ".TABLE_MAGNA_TRADORIA_PREPARE." WHERE mpID='".$this->getMagnaSession('mpID')."' AND ".$sKeyType." in('".implode("', '", $aVariants)."') AND PrepareType=\'Match\'")
					) {
						MagnaDB::gi()->delete(TABLE_MAGNA_TRADORIA_PREPARE, array(
							'mpID' => $this->getMagnaSession('mpID'),
