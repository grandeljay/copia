...
class MLProductListDependencyTradoriaApplyFormAction extends MLProductListDependency {
	public function getActionBottomLeftTemplate(){
		return 'tradoriaapplyformleft';
	}
	
	public function getActionBottomRightTemplate(){
		return 'tradoriaapplyformright';
	}
	...
	protected function removeApply() {
		...
		if (!empty($pIDs)) {
			foreach ($pIDs as $pID) {
				$aProduct = MLProduct::gi()->setLanguage(getDBConfigValue('tradoria.lang', $this->getMagnaSession('mpID'), $_SESSION['magna']['selected_language']))->getProductById($pID);
				...
				MagnaDB::gi()->delete(TABLE_MAGNA_TRADORIA_PREPARE, $where, $sAdd);
	...
	protected function resetApply() {
		...
		if (!empty($pIDs)) {
			...
			'	);
				...
				$aProducts = MagnaDB::gi()->fetchArray('
					SELECT p.products_id AS PID,
					       ...
					       aa.*
					  FROM '.TABLE_MAGNA_TRADORIA_PREPARE.' aa
			...
			foreach ($aProducts as $aRow) {				
				...
				MagnaDB::gi()->update(TABLE_MAGNA_TRADORIA_PREPARE, array (
