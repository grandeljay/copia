...
abstract class MLProductListTradoriaAbstract extends MLProductList {
	...
	protected function isPreparedDifferently($aRow) {
		$sPrimaryCategory = $this->getPrepareData($aRow, 'MarketplaceCategoriesName');
		if (!empty($sPrimaryCategory)) {
			$sCategoryDetails = $this->getPrepareData($aRow, 'CategoryAttributes');
			$categoryMatching = TradoriaHelper::gi()->getCategoryMatching($sPrimaryCategory);
			$categoryDetails = json_decode($sCategoryDetails, true);
			return TradoriaHelper::gi()->detectChanges($categoryMatching, $categoryDetails);
		}
	...
	protected function isDeletedAttributeFromShop($aRow, &$message) {
        $aMarketplaceCategoriesName = $this->getPrepareData($aRow, 'MarketplaceCategoriesName');
		if (!empty($aMarketplaceCategoriesName)) {
			$matchedAttributes = $this->getPrepareData($aRow, 'CategoryAttributes');
			$matchedAttributes = json_decode($matchedAttributes, true);
			$shopAttributes = TradoriaHelper::gi()->flatShopVariations();
            ...
            foreach ($matchedAttributes as $matchedAttribute) {
				if (TradoriaHelper::gi()->detectIfAttributeIsDeletedOnShop($shopAttributes, $matchedAttribute, $message)) {
					return true;
				}
			}
		}
