...
class TradoriaTopTenCategories extends TopTen {
	...
	public function getTopTenCategories($sType, $sGetCatPathFunc = 'getMPCategoryPath'){
		...
		if (empty($aTopTenCat)) {
			$aTopTenCat = MagnaDB::gi()->fetchArray('
				SELECT DISTINCT MpIdentifier
				  FROM '.TABLE_MAGNA_TRADORIA_VARIANTMATCHING.'
		...
		$oDCM = new TradoriaCategoryMatching();
		...
		foreach ($aTopTenCat as $iCatId) {
			...
			if (strpos($aTopTenCatIds[$iCatId], '"invalid"') !== false) {
				...
				// no mpid
				MagnaDB::gi()->query('
					UPDATE '.TABLE_MAGNA_TRADORIA_PREPARE.'
					   SET '.$sType.' = 0
					 WHERE '.$sType.' = "'.$iCatId.'"
				');
			}
