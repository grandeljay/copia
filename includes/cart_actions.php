  ...
  // do not redirect to shopping cart if delete from box cart
    if (isset($_GET['box']) && $_GET['box'] == 'cart' && isset($_GET['prd_id'])) {
        ...
        for ($i = 0, $n = count($parameters); $i < $n; $i++) {
            if (in_array($parameters[$i], array('products_id', 'cPath'))) {
                unset($parameters[$i]);
            }
        }
    }
    ...
    switch ($action) {
        case 'remove_product':
            foreach (auto_include(DIR_FS_CATALOG . 'includes/extra/cart_actions/remove_product_prepare_get/', 'php') as $file) {
                require($file);
            }
            $prd_id = xtc_input_validation($_GET['prd_id'], 'products_id');
            $cart_object->remove($prd_id);
            foreach (auto_include(DIR_FS_CATALOG . 'includes/extra/cart_actions/remove_product_before_redirect/', 'php') as $file) {
                require($file);
            }
      ...
      // customer wants to update the product quantity in their shopping cart
        case 'update_product':
            foreach (auto_include(DIR_FS_CATALOG . 'includes/extra/cart_actions/update_product_prepare_post/', 'php') as $file) {
                require($file);
            }
          ...
          // check gift
            ...
            foreach (auto_include(DIR_FS_CATALOG . 'includes/extra/cart_actions/update_product_before_redirect/', 'php') as $file) {
                require($file);
            }
      ...
      // customer adds a product from the products page
        case 'add_product':
            foreach (auto_include(DIR_FS_CATALOG . 'includes/extra/cart_actions/add_product_prepare_post/', 'php') as $file) {
                require($file);
            }
            if (
            ...
            foreach (auto_include(DIR_FS_CATALOG . 'includes/extra/cart_actions/add_product_before_redirect/', 'php') as $file) {
                require($file);
            }
            if ($co_express === true) {
      ...
      // customer wants to add a quickie to the cart (called from a box)
        case 'add_a_quickie':
            foreach (auto_include(DIR_FS_CATALOG . 'includes/extra/cart_actions/add_a_quickie_prepare_post/', 'php') as $file) {
                require($file);
            }
      ...
      // performed by the 'buy now' button in product listings and review page
        case 'buy_now':
            foreach (auto_include(DIR_FS_CATALOG . 'includes/extra/cart_actions/buy_now_prepare_get/', 'php') as $file) {
                require($file);
            }
      ...
      ## Paypal
        ...
        case 'custom':
            foreach (auto_include(DIR_FS_CATALOG . 'includes/extra/cart_actions/custom/', 'php') as $file) {
                require($file);
            }
            break;
    }
