...
/* -----------------------------------------------------------------------------------------
   $Id: application_bottom.php 11987 2019-07-23 06:06:00Z GTB $
...
} elseif (DISPLAY_PAGE_PARSE_TIME == 'admin' && $_SESSION['customers_status']['customers_status'] == '0') {
    $parse_time = number_format((microtime(true) - PAGE_PARSE_START_TIME), 3);
    echo '<div class="parseTime">Parse Time: ' . $parse_time . 's</div>';
}
...
foreach (auto_include(DIR_FS_CATALOG . 'includes/extra/application_bottom/', 'php') as $file) {
    require($file);
}
