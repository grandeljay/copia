...
/* -----------------------------------------------------------------------------------------
   $Id: shopping_cart.php 14045 2022-02-11 10:12:32Z GTB $
...
if ($_SESSION['cart']->count_contents() > 0) {
    $smarty->assign('FORM_ACTION', xtc_draw_form('cart_quantity', xtc_href_link(FILENAME_SHOPPING_CART, 'action=update_product', $request_type)));
    ...
    if (STOCK_CHECK == 'true') {
        if (1 == $_SESSION['any_out_of_stock']) {
...
if (defined('MODULE_CHECKOUT_EXPRESS_STATUS') && MODULE_CHECKOUT_EXPRESS_STATUS == 'true') {
    if (isset($_SESSION['customer_id']) && DEFAULT_CUSTOMERS_STATUS_ID_GUEST != $_SESSION['customers_status']['customers_status_id']) {
        $express_query = xtc_db_query(
            "SELECT *
