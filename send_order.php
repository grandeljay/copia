...
if ($_SESSION['customer_id'] == $order_check['customers_id'] || $send_by_admin) { // Send Order by Admin
    ...
    if (in_array($order->info['payment_method'], $payment_method_array)) {
        $payment_text = defined('MODULE_PAYMENT_' . strtoupper($order->info['payment_method']) . '_TEXT_DESCRIPTION')
                      ? sprintf(constant('MODULE_PAYMENT_' . strtoupper($order->info['payment_method']) . '_TEXT_DESCRIPTION'), $insert_id)
                      : '';
    ...
    $smarty->assign(
        'img_path',
        HTTP_SERVER . DIR_WS_CATALOG . DIR_WS_IMAGES . 'product_images/' .
        (defined('SHOW_IMAGES_IN_EMAIL_DIR') ? SHOW_IMAGES_IN_EMAIL_DIR : 'thumbnail') .
        '_images/'
    );
    ...
    foreach (auto_include(DIR_FS_CATALOG . 'includes/extra/send_order/data/', 'php') as $file) {
        require $file;
    }
    ...
    foreach (auto_include(DIR_FS_CATALOG . 'includes/extra/send_order/mail/', 'php') as $file) {
        require $file;
    }
